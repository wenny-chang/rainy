    ...
    (L.IdwLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        _redraw: function () {
            ...
            for (var k = 0; k < numberOfData; k++) {
                ...
                if (isNaN(this._latlngs[k][dataType]) || this._latlngs[k][dataType] > maxVal || this._latlngs[k][dataType] < minVal) {
                    // console.log("malfunction station: ", this._latlngs[k]);
                    continue;
                }
                // eliminate data outside the screen
                if (
                    this._latlngs[k][0] > leftTop.lat ||
            ...
            for (var i = 0; i < nCellY; i++) {
                for (var j = 0; j < nCellX; j++) {
                    ...
                    // IDW value
                    var cell = [Math.round(j * r), Math.round(i * r), Math.min(interpolVal, maxVal)];
